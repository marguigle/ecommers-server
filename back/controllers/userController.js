...
export const userCart = expressAsyncHandler(async (req, res) => {
  console.log("userCart called with body:", req.body);
  const { cart } = req.body;
  const { _id } = req.user;
  ...
  try {
    ...
    if (alreadyExistCart) {
      console.log("Removing existing cart");
    ...
    for (let i = 0; i < cart.length; i++) {
      ...
      console.log("Found product:", product?._id, "price:", product?.price);
      if (!product) {
        res.status(404);
    ...
    console.log("Cart saved:", newCart._id);
    res.json(newCart);
  } catch (error) {
